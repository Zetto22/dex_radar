#!/usr/bin/env python3
"""Build GitHub Release notes from a mod CHANGELOG.md (Dramatic Shape style).

Usage:
  python tools/github_release_notes.py dex_radar 1.0.0
  python tools/github_release_notes.py dex_radar 1.0.0 --title-only

Stdout:
  default  → full release body (install blurb + ## Changes)
  --title-only → "<version> - <headline>"
"""
...
def section_for(changelog: str, version: str) -> str | None:
    for match in HEADING_RE.finditer(changelog):
        ...
        # Allow "1.0.0" to match heading "1.0.0" or "1.0.0 - something"
...
def headline(body: str, version: str) -> str:
    ...
    # First prose paragraph before any ### heading / bullet list.
    prose: list[str] = []
    ...
    if prose:
        text = prose[0]
        # One short sentence for the release title.
...
def change_bullets(body: str) -> list[str]:
    ...
    for line in body.splitlines():
        ...
        if stripped.startswith("- "):
            ...
            continue
        # Indented continuation of the previous bullet (Keep a Changelog wrap).
...
def main() -> int:
    ...
    path = args.root / args.mod_id / "CHANGELOG.md"
    if not path.is_file():
